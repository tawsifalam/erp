...
export function ReservationInclusionsContent({
  tenant,
  branchId,
  reservationId,
}: {
  ...
  return (
    <Stack gap={3}>
      {allowances.map((a) => (
        <Box key={a.id} p={3} borderWidth="1px" borderRadius="md" width="100%">
          ...
          {a.inclusionType === InclusionType.MEAL && a.remainingQty > 0 && (
            <Button size="sm" mt={2} onClick={() => recordMeal(a)}>
              Record comp meal (1)
            </Button>
          )}
          ...
          {a.inclusionType === InclusionType.AMENITY_KIT && a.remainingQty > 0 && (
            <Button size="sm" mt={2} variant="outline" onClick={() => reissueKit(a)}>
